...
async function getCBTResults(
  id: any,
  exam: any
): Promise<ServerResponse<TeacherCBTResultsGETData>> {
  await connect();
  let [success, status, message]: ServerResponse<TeacherCBTResultsGETData> = [
    false,
...
export default async function handler(
  ...
  if (allowedMethods !== method) {
    ...
    [success, status, message] = await getCBTResults(query.id, query.examId);

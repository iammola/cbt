...
async function getEventsRange(from: any, to: any): Promise<ServerResponse<EventsRangeGETData>> {
    ...
    try {
        ...
        }, 'from exams').populate('exams', 'subjectId').lean();

        const data = await Promise.all(events.map(async ({ from, exams }) => ({
            date: new Date(from),
            time: format(new Date(from), 'h:mm a'),
            events: (await Promise.all(exams.map(async ({ subjectId }) => {
                const record: SubjectsRecord<true> | null = await SubjectsModel.findOne({
                    "subjects._id": subjectId
                }, 'class subjects.name.$').populate('class', 'alias').lean();

...
async function getClassStudents(classId: any): Promise<ServerResponse<ClassStudentsGETData>> {
    await connect();
    let [success, status, message]: ServerResponse<ClassStudentsGETData> = [false, StatusCodes.INTERNAL_SERVER_ERROR, ReasonPhrases.INTERNAL_SERVER_ERROR];

    try {
        const [record, currentSession] = await Promise.all([
            SubjectsModel.findOne({ class: classId }, '-_id subjects._id').lean(),
            SessionModel.findOne({ current: true, "terms.current": true }, 'terms._id.$').lean(),
        ]);
                ...
                $elemMatch: {
                    ...
                    terms: {
                        $elemMatch: {
                            ...
                            subjects: { $in: record?.subjects.map(subject => subject._id) },
                        }
                    }
                }
            }

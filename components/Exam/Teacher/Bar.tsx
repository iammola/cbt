...
const Bar: FunctionComponent<TeacherBarProps> = ({
  exam,
  save,
  modified,
  saved,
  ...
  const { data: currentSession } = useSWRImmutable<
    RouteData<SessionCurrentGETData>
  >("/api/sessions/current/", (url) => fetch(url).then((res) => res.json()));
  ...
  return (
    <div className="flex items-center justify-end gap-6 w-full bg-white py-5 px-8 sticky left-0 top-0 rounded-b-lg drop-shadow-sm">
      <div className="hidden md:flex items-center justify-start gap-2 grow text-gray-400 w-full text-sm font-medium">
        ...
        <span className="w-max block truncate">
          {exam?.class ?? "Select Class"}
        </span>
        <ChevronRightIcon className="w-5 h-5 fill-gray-500" />
        <span className="w-max block truncate">
          {exam?.subject ?? "Select Subject"}
        </span>
        <ChevronRightIcon className="w-5 h-5 fill-gray-500" />
      ...
      <button
        ...
        title={
          modified === true
            ? "Changes made. Don't forget to save"
            : saved === true
            ? "No changes made."
            : ""
        }
      ...
      >
        ...
        {saved === true && <CheckIcon className="w-5 h-5 fill-indigo-700" />}
        Save
      </button>
      <button
        type="submit"
      ...
      >
        ...
        {uploaded === true && <CheckIcon className="w-5 h-5 fill-white" />}
        Submit
      </button>
    </div>
  );
